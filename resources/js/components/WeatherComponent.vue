<template>
  <div class="text-white mb-8">
    <div class="places-input text-gray-800">
      <input type="text" id="address" placeholder="Choose a city..."/>
    ...
    <div
        class="weather-container font-sans md:w-128 max-w-lg rounded-lg overflow-hidden bg-gray-900 shadow-lg mt-8 drop-shadow-2xl"
    >
      <div class="current-weather flex items-center justify-between px-6 py-8">
        ...
        <div>
          <img :src="loading ? loadingIcon : currentTemp.icon" width="90px" height="90px"/>
        </div>
      </div>
      <!-- end current-weather -->

      <div class="future-weather text-sm bg-gray-800 px-6 py-8 overflow-hidden pt-0">
        <div
        ...
        >
          <div class="w-1/6 text-lg text-gray-200 font-light">{{ toDayOfWeek(day.dt) }}</div>
          <div class="w-4/6 px-4 flex items-center">
            <div>
              <img :src="iconLink + day.weather[0].main.toLowerCase() + '.svg'" width="70px" height="70px"/>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      daily: [],
      loadingIcon: "/images/icons/loading.svg",
      iconLink: "/images/icons/"
    };
  },
  methods: {
    fetchData() {
      ...
      fetch(`/api/weather?lat=${this.location.lat}&lon=${this.location.lon}`)
          ...
          .then(data => {
            ...
            this.currentTemp.icon = this.iconLink + data.current.weather[0].main.toLowerCase() + ".svg";
    ...
    fetchLocation() {
      var placesAutocomplete = places({
        appId: Weathy.places.appId,
        apiKey: Weathy.places.apiKey,
